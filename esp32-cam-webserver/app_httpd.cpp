...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    if(!strcmp(variable, "framesize")) {
        ...
        esp_task_wdt_init(&twdt_config);  // schedule a watchdog panic event for 3 seconds in the future
        esp_task_wdt_add(NULL);
        
        // These functions are no longer available in ESP-IDF 5.x
        // Try to use ledc functions to shut down peripherals
        ledc_stop(LEDC_HIGH_SPEED_MODE, (ledc_channel_t)lampChannel, 0);
